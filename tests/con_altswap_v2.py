...
def addLiquidity(poolName: str, assets: dict):
    ...
    for contract, amount in assets.items():
        ...
        # This is the same check as the next line, just shorter and easier to understand. Do we want to use that?
        # assert pools[poolName, contract, 'weight'] == amount / sum_assets, f"Amount and weights for {contract} not matching"
...
def removeLiquidity(poolName: str, amountLPTokens: float):
    ...
    total_lp_points = lp_tokens[poolName]
